<div>
    ...
    <nav class="mb-4 flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
        <a href="{{ route('portal.kb.index') }}" wire:navigate class="hover:text-brand-purple">Help</a>
        <span>/</span>
    ...
    <div class="space-y-2">
        @forelse ($articles as $article)
            <a href="{{ route('portal.kb.article', $article->slug) }}" wire:navigate class="block rounded-lg border border-gray-200 bg-white p-4 hover:border-brand-purple/40 dark:border-ink-600 dark:bg-ink-850">
                ...
                <p class="mt-1 text-xs text-gray-400">{{ number_format($article->view_count) }} views · {{ $article->published_at?->format('M d, Y') }}</p>
            </a>
        @empty
            <div class="rounded-lg border border-gray-200 bg-white p-10 text-center text-sm text-gray-400 dark:border-ink-600 dark:bg-ink-850">No articles in this category yet.</div>
        @endforelse
    </div>
