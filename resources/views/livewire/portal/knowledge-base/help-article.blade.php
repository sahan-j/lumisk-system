<div>
    ...
    <nav class="mb-4 flex flex-wrap items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span>/</span>
        <a href="{{ route('portal.kb.category', $article->category->slug) }}" wire:navigate class="hover:text-brand-purple">{{ $article->category->name }}</a>
        <span>/</span>
    ...
    <article class="mx-auto max-w-3xl">
        ...
        <header class="mb-6 border-b border-gray-200 pb-5 dark:border-ink-600">
            ...
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-400">
                ...
                @if ($article->author_name)<span>·</span><span>By {{ $article->author_name }}</span>@endif
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    @foreach ($related as $rel)
                        <a href="{{ route('portal.kb.article', $rel->slug) }}" wire:navigate class="rounded-lg border border-gray-200 bg-white p-3 text-sm font-medium text-gray-700 hover:border-brand-purple/40 dark:border-ink-600 dark:bg-ink-850 dark:text-gray-200">{{ $rel->title }}</a>
                    @endforeach
                </div>
            </div>
        @endif
