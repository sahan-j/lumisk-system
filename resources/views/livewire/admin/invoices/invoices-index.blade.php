<div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Create and manage invoices.</p>
        </div>
    ...
    @if (count($selected) > 0)
        <div class="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-brand-purple/50 bg-brand-purple/5 px-4 py-2.5">
            ...
            <button wire:click="bulkMarkPaid" class="btn-secondary !px-3 !py-1 text-xs">Mark Paid</button>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-ink-600">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-ink-700">
                    @forelse ($invoices as $invoice)
                        <tr wire:key="invoice-{{ $invoice->id }}" class="hover:bg-gray-50 dark:hover:bg-ink-800">
                            <td class="px-5 py-3">
                                <input type="checkbox" wire:model.live="selected" value="{{ $invoice->id }}" class="rounded border-gray-300 text-brand-purple focus:ring-brand-purple dark:border-ink-600 dark:bg-ink-800">
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-right text-sm font-medium text-gray-900 dark:text-white">
                                {{ currency_amount($invoice, $invoice->total) }}
                                @if ($invoice->currency_code !== 'LKR')<div class="text-[10px] font-normal text-gray-400">≈ {{ money($invoice->total_lkr) }}</div>@endif
                            </td>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.invoices.edit', $invoice) }}" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-ink-700" title="Edit">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </a>
                                    <a href="{{ route('admin.invoices.pdf', $invoice) }}" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-ink-700" title="Download PDF">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                    </a>
                                    <button wire:click="$dispatch('open-duplicate', { type: 'invoice', id: {{ $invoice->id }} })" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-ink-700" title="Duplicate">
                                    ...
                                    <button wire:click="$dispatch('open-convert', { direction: 'invoice_to_estimate', id: {{ $invoice->id }} })" class="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-ink-700" title="Convert to Estimate">
                                    ...
                                    <button wire:click="confirmDelete({{ $invoice->id }})" class="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30" title="Delete">
                    ...
                    @empty
                        <tr><td colspan="9" class="px-5 py-12 text-center text-sm text-gray-400">No invoices found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    @if ($confirmingDelete)
        <x-app-modal title="Delete invoice?" close="$set('confirmingDelete', false)">
            <p class="text-sm text-gray-600 dark:text-gray-300">This will soft-delete the invoice. This action can be reversed from the database.</p>
