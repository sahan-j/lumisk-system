  ...
  <div class="body">
    ...
    <div class="detail-box">
      <div class="detail-row">
        ...
        <span class="detail-value">{{ $estimate->estimate_number }}</span>
      </div>
      <div class="detail-row">
        <span class="detail-label">Issue Date</span>
        <span class="detail-value">{{ $estimate->issue_date->format('M d, Y') }}</span>
      </div>
      @if($estimate->expiry_date)
      <div class="detail-row">
        <span class="detail-label">Expiry Date</span>
        <span class="detail-value">{{ $estimate->expiry_date->format('M d, Y') }}</span>
      </div>
      @endif
      <div class="detail-row total-row">
        <span class="detail-label">Total Amount</span>
        <span class="detail-value">{{ $company->currency ?: 'LKR' }} {{ number_format((float)$estimate->total, 2) }}</span>
      </div>
    </div>
  </div>
  <div class="footer">
