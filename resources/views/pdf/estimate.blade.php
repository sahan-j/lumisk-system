...
@php
    $currCode = $estimate->currency_code ?: ($company->currency ?: 'LKR');
    $currency = $estimate->currency_symbol;
...
<div class="cf">
    ...
    <div class="hd-right">
        <div class="doc-word">ESTIMATE</div>
        <div class="doc-num mono">{{ $estimate->estimate_number }}</div>
        <div><span class="pill">{{ strtoupper($estimate->status) }}</span></div>
    </div>
</div>
...
<div class="cf">
    <div class="bill-left">
        ...
        <div class="cname">{{ $estimate->client->name }}</div>
        @if($estimate->client->company_name)<div class="cmeta">{{ $estimate->client->company_name }}</div>@endif
        @if($estimate->client->email)<div class="cmeta">{{ $estimate->client->email }}</div>@endif
        @if($estimate->client->phone)<div class="cmeta">{{ $estimate->client->phone }}</div>@endif
        @if($estimate->client->address)<div class="cmeta">{{ $estimate->client->address }}</div>@endif
    </div>
    <div class="bill-right">
        <table class="dates">
            <tr>
                <td class="k">Issue Date</td>
                <td class="v mono">{{ $estimate->issue_date->format('Y-m-d') }}</td>
            </tr>
            <tr>
                <td class="k">Valid Until</td>
                <td class="v mono">{{ $estimate->expiry_date ? $estimate->expiry_date->format('Y-m-d') : '—' }}</td>
            </tr>
        </table>
    </div>
</div>
    ...
    <div class="totals">
        <table class="tt">
            <tr>
                ...
                <td class="tv mono">{{ $currency }} {{ number_format((float)$estimate->subtotal, 2) }}</td>
            </tr>
            <tr>
                <td class="tl">Tax ({{ rtrim(rtrim(number_format((float)$estimate->tax_rate, 2), '0'), '.') }}%)</td>
                <td class="tv mono">{{ $currency }} {{ number_format((float)$estimate->tax_amount, 2) }}</td>
            </tr>
            @if($estimate->discount_amount > 0)
            <tr>
                <td class="tl">Discount</td>
                <td class="tv mono" style="color:#00a8cc">- {{ $currency }} {{ number_format((float)$estimate->discount_amount, 2) }}</td>
            </tr>
            @endif
        </table>
        <div class="grand">
            <table class="grand-tbl">
                <tr>
                    ...
                    <td class="gv mono">{{ number_format((float)$estimate->total, 2) }}</td>
                </tr>
            </table>
        </div>
        @if($isForeign)
        <div style="text-align:right; font-size:0.8em; color:#94a3b8; margin-top:5px;">
            &asymp; Rs {{ number_format((float)$estimate->total_lkr, 2) }} at 1 {{ $currCode }} = Rs {{ number_format((float)$estimate->exchange_rate, 2) }}
        </div>
        @endif
    </div>
</div>
...
@if($hasNotes || $hasBank || $estimate->terms)
<div class="page-two">
    @if($hasNotes || $hasBank)
    <div class="cf">
        @if($hasBank && $hasNotes)
            ...
            <div class="col-right">
                ...
                <div class="body-text">{!! nl2br(e($estimate->notes)) !!}</div>
            </div>
        @elseif($hasBank)
            <div class="slabel">Payment Details</div>
    ...
    @if($estimate->terms)
        ...
        <div>
            ...
            <div class="body-text">{!! nl2br(e($estimate->terms)) !!}</div>
        </div>
    @endif
</div>
@endif
