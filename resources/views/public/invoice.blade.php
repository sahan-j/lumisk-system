...
<head>
  ...
  <title>Invoice {{ $record->invoice_number }} — {{ $company->name }}</title>
  <style>
...
<div class="container">
  ...
  <div class="header">
    ...
    <div style="text-align:right">
      <div style="font-size:11px;color:#6d5cff;font-weight:600;text-transform:uppercase;letter-spacing:1px">Invoice</div>
      <div style="font-size:22px;font-weight:bold;">{{ $record->invoice_number }}</div>
  ...
  <div class="meta">
    ...
    <div style="text-align:right">
      ...
      <div class="muted">Due Date: <strong>{{ $record->due_date?->format('M d, Y') ?? '—' }}</strong></div>
    </div>
  </div>
